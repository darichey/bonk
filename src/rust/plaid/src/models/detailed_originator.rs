/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.586.4
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// DetailedOriginator : Originator and their status.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct DetailedOriginator {
    /// Originator’s client ID.
    #[serde(rename = "client_id")]
    pub client_id: String,
    #[serde(rename = "transfer_diligence_status")]
    pub transfer_diligence_status: models::TransferDiligenceStatus,
    /// The company name of the end customer.
    #[serde(rename = "company_name")]
    pub company_name: String,
    /// List of outstanding requirements for scaled platform originators. Only populated when `transfer_diligence_status` is `more_information_required`.
    #[serde(rename = "outstanding_requirements", skip_serializing_if = "Option::is_none")]
    pub outstanding_requirements: Option<Vec<models::TransferPlatformRequirement>>,
}

impl DetailedOriginator {
    /// Originator and their status.
    pub fn new(client_id: String, transfer_diligence_status: models::TransferDiligenceStatus, company_name: String) -> DetailedOriginator {
        DetailedOriginator {
            client_id,
            transfer_diligence_status,
            company_name,
            outstanding_requirements: None,
        }
    }
}

