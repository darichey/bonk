/*
 * The Plaid API
 *
 * The Plaid REST API. Please see https://plaid.com/docs/api for more details.
 *
 * The version of the OpenAPI document: 2020-09-14_1.586.4
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// BankTransfer : Represents a bank transfer within the Bank Transfers API.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct BankTransfer {
    /// Plaid’s unique identifier for a bank transfer.
    #[serde(rename = "id")]
    pub id: String,
    #[serde(rename = "ach_class")]
    pub ach_class: models::AchClass,
    /// The account ID that should be credited/debited for this bank transfer.
    #[serde(rename = "account_id")]
    pub account_id: String,
    #[serde(rename = "type")]
    pub r#type: models::BankTransferType,
    #[serde(rename = "user")]
    pub user: models::BankTransferUser,
    /// The amount of the bank transfer (decimal string with two digits of precision e.g. \"10.00\").
    #[serde(rename = "amount")]
    pub amount: String,
    /// The currency of the transfer amount, e.g. \"USD\"
    #[serde(rename = "iso_currency_code")]
    pub iso_currency_code: String,
    /// The description of the transfer.
    #[serde(rename = "description")]
    pub description: String,
    /// The datetime when this bank transfer was created. This will be of the form `2006-01-02T15:04:05Z`
    #[serde(rename = "created")]
    pub created: String,
    #[serde(rename = "status")]
    pub status: models::BankTransferStatus,
    #[serde(rename = "network")]
    pub network: models::BankTransferNetwork,
    /// When `true`, you can still cancel this bank transfer.
    #[serde(rename = "cancellable")]
    pub cancellable: bool,
    #[serde(rename = "failure_reason", deserialize_with = "Option::deserialize")]
    pub failure_reason: Option<models::BankTransferFailure>,
    /// A string containing the custom tag provided by the client in the create request. Will be null if not provided.
    #[serde(rename = "custom_tag", deserialize_with = "Option::deserialize")]
    pub custom_tag: Option<String>,
    /// The Metadata object is a mapping of client-provided string fields to any string value. The following limitations apply: The JSON values must be Strings (no nested JSON objects allowed) Only ASCII characters may be used Maximum of 50 key/value pairs Maximum key length of 40 characters Maximum value length of 500 characters 
    #[serde(rename = "metadata", deserialize_with = "Option::deserialize")]
    pub metadata: Option<std::collections::HashMap<String, String>>,
    /// Plaid’s unique identifier for the origination account that was used for this transfer.
    #[serde(rename = "origination_account_id")]
    pub origination_account_id: String,
    #[serde(rename = "direction", deserialize_with = "Option::deserialize")]
    pub direction: Option<models::BankTransferDirection>,
}

impl BankTransfer {
    /// Represents a bank transfer within the Bank Transfers API.
    pub fn new(id: String, ach_class: models::AchClass, account_id: String, r#type: models::BankTransferType, user: models::BankTransferUser, amount: String, iso_currency_code: String, description: String, created: String, status: models::BankTransferStatus, network: models::BankTransferNetwork, cancellable: bool, failure_reason: Option<models::BankTransferFailure>, custom_tag: Option<String>, metadata: Option<std::collections::HashMap<String, String>>, origination_account_id: String, direction: Option<models::BankTransferDirection>) -> BankTransfer {
        BankTransfer {
            id,
            ach_class,
            account_id,
            r#type,
            user,
            amount,
            iso_currency_code,
            description,
            created,
            status,
            network,
            cancellable,
            failure_reason,
            custom_tag,
            metadata,
            origination_account_id,
            direction,
        }
    }
}

